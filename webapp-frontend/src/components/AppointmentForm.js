...
export default function AppointmentForm({ timeSlot, onSubmit, onCancel, loading }) {
    ...
    return (
        <>
            <div className="rounded bg-blue-200 p-3">
                <form onSubmit={(e) => onSubmit(e, patientDescription, insuranceType)}>
                    ...
                    <button
                        className="w-full bg-blue-500 text-lg text-white py-2 mt-8"
                        type="submit"
                        disabled={loading}
                    >
                        {loading ? "..." : "Make Appointment"}
                    </button>
                </form>
                <button onClick={onCancel} disabled={loading} className="w-full bg-gray-500 text-lg text-white py-2 mt-3">Back</button>
            </div>
        </>
    );
}